...
function exportTeamReports() {
  ...
  // Per-team breakdown lines
  const teamBreakdown = teamList.map(team => {
    const tr         = byTeam[team];
    const active     = tr.filter(r => !r.onLeave);
    const used       = active.filter(r => r.used).length;
    const notUsed    = active.filter(r => !r.used).length;
    const onLeave    = tr.filter(r => r.onLeave).length;
    const pct        = active.length ? Math.round((used / active.length) * 100) : 0;
    return `  ${team}: ${used}/${active.length} used (${pct}%)${onLeave ? `, ${onLeave} on leave` : ""}${notUsed ? ` — ${notUsed} not used` : ""}`;
  }).join("\n");
  ...
  const notUsedBlock = notUsedSections.length
    ? `The following practitioners have NOT yet used ICA today:\n\n${notUsedSections.join("\n\n")}`
    : `All active practitioners have used ICA today.`;

  const body = encodeURIComponent(
    `Dear Managers,\n\n` +
    `Please find the ICA adoption summary for ${dateKey} below.\n\n` +
    `================================================================\n` +
    `OVERALL SUMMARY\n` +
    `================================================================\n` +
    `  Total active practitioners : ${totalActive}\n` +
    `  Used ICA today             : ${totalUsed} (${overallPct}%)\n` +
    `  Not used                   : ${totalNotUsed}\n` +
    ...
    `${teamBreakdown}\n\n` +
    `================================================================\n` +
    `PRACTITIONERS NOT USING ICA\n` +
    `================================================================\n` +
    `${notUsedBlock}\n\n` +
    `Please encourage the above practitioners to use ICA.\n` +
    `Access ICA here: https://remea.ica.ibm.com/ica/curatorai/apps/ui/new-chat\n\n` +
    `----------------------------------------------------------------\n` +
    `For full details, charts and team-wise drill-down, view the live dashboard:\n` +
